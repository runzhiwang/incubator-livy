  ...
  with GZipSupport
{
   ...
   */
  protected def createSession(req: HttpServletRequest): Option[S]
  ...
  post("/") {
    synchronized {
      if (tooManySessions) {
        ...
        val createdSession = createSession(request)
        if (createdSession.isEmpty) {
          BadRequest(ResponseMessage("Rejected, fail to create livy session!"))
        }

        val session = sessionManager.register(createdSession)
